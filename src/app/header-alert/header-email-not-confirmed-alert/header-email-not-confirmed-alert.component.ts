...
export class HeaderEmailNotConfirmedAlertComponent implements OnInit {
	...
	constructor(private _userService: UserService, private _authLoginService: AuthLoginService) {
	...
	private checkIfEmailIsValidated() {
		this._userService.getUserDetail().then((userDetail: UserDetail) => {
			...
			console.log('HeaderEmailNotConfirmedAlert: could not get userDetail', getUserDetailError);
		});
	}
