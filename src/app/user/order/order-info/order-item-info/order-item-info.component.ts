...
import { OrderItem, CustomerItem, BlApiError } from "@boklisten/bl-model";
...
export class OrderItemInfoComponent implements OnInit {
	...
	ngOnInit() {
		if (
			this.orderItem &&
			this.orderItem.type === "rent" &&
			this.orderItem.info.customerItem
		) {
			this._customerItemService
				.getById(this.orderItem.info.customerItem as string)
				.then((customerItem: CustomerItem) => {
					this.customerItem = customerItem;
				})
				.catch((customerItemError: BlApiError) => {
					console.log(
						"orderITemInfoComponent: error when trying to get orderItem",
						customerItemError
					);
				});
		}
