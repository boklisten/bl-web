...
export class BranchOpeningHoursComponent implements OnInit, OnChanges {
	...
	constructor(
		private _branchOpeningHoursService: BranchOpeningHoursService,
	...
	ngOnChanges() {
		...
		if (
			!this.branch ||
			!this.branch.openingHours ||
			this.branch.openingHours.length <= 0
		) {
			return;
		}
		...
		this._branchOpeningHoursService
			.getOpeningHours(this.branch)
			.then((openingHours: OpeningHour[]) => {
				this.openingHours = this.removeInactiveOpeningHours(
					openingHours
				);
	...
	private removeInactiveOpeningHours(
